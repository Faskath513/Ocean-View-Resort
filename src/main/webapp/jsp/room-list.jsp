            ...
            <div class="dashboard-grid fade-in">
                <c:forEach var="room" items="${listRooms}">
                    <div class="dashboard-card">
                        <!-- Room Image -->
                        <div class="bg-primary-soft d-flex align-items-center justify-content-center" style="height:150px; border-radius: var(--radius); margin-bottom: 15px;">
                            <i class="bi bi-door-closed" style="font-size:2.5rem; color: var(--primary);"></i>
                        </div>

                        <!-- Room Info -->
                        <h5 class="text-primary">${room.roomNumber}</h5>
                        <p class="text-muted mb-1">${room.type}</p>
                        <p class="fw-bold text-info mb-2">$${room.pricePerNight} <small class="text-muted">/night</small></p>
                        <div class="mb-3">
                            <c:choose>
                        ...
                        <div class="d-flex gap-2">
                            <a href="${pageContext.request.contextPath}/rooms?action=edit&id=${room.id}" class="btn btn-outline-primary btn-sm flex-fill">
                                <i class="bi bi-pencil"></i> Edit
                            </a>
                            <a href="${pageContext.request.contextPath}/rooms?action=delete&id=${room.id}" class="btn btn-outline-danger btn-sm flex-fill"
